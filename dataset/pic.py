...
class PICDataSet(data.Dataset):
    def __init__(self, root, dataset, crop_size=[473, 473], scale_factor=0.25,
        ...
        self.im_list = [line.strip().rstrip('\n').split(' ')[0] for line in open(self.file_list_name).readlines()]
    ...
    def __getitem__(self, index):
        ...
        edge_path = im_path.replace('image', 'edge').replace('jpg', 'png')
        parsing_anno_path = im_path.replace('image', 'seg').replace('jpg', 'png')

        im = cv2.imread(im_path, cv2.IMREAD_COLOR)
        h, w, _ = im.shape
        parsing_anno = np.zeros((h, w), dtype=np.long)
        edge = np.zeros((h, w), dtype=np.long)
