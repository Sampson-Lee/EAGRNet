...
glob_start = timeit.default_timer()
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            if not dist.is_initialized() or dist.get_rank() == 0:
                if (i_iter+1) % 50 == 0:
                    ...
                    glob_t_intv = end - glob_start
                    eta = int((total_iters - i_iter) * (glob_t_intv / i_iter))
                    glob_t_intv = str(datetime.timedelta(seconds=glob_t_intv))
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            ...
            torch.save(state_dict, osp.join(args.snapshot_dir, TIMESTAMP, 'epoch_' + str(epoch) + '.pth'))

            # if (epoch+1) % args.test_fre == 0:
            ...
            #         model.cpu()
            #         state_dict = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
            #         torch.save(state_dict, osp.join(args.snapshot_dir, TIMESTAMP, 'best.pth'))
            #         best_f1 = f1['overall']
            #     print(mIoU)
            #     print(f1)
    ...
    glob_end = timeit.default_timer()
    print(glob_end - glob_start, 'seconds')
